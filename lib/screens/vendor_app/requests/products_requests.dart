...
class VendorAppProductsReq extends HttpService {
  ...
  Future<List<StoreProduct>> getStoreProducts() async {
    ...
    var list = List<StoreProduct>.from(apiResult.data["store"]["store_products"]
        .map((x) => StoreProduct.fromJson(x)));
  ...
  Future<bool> changeProductStatus(int productId) async {
    ...
    try {
      final apiResult = await postRequest(Api.changeProductStatus, formData,
          includeHeaders: true);
  ...
  Future<List<ProductType>> getStoreTypes() async {
    ...
    return List<ProductType>.from(
        apiResult.data["product_types"].map((x) => ProductType.fromJson(x)));
    // StoreModel storeModel= storeModelFromJson(apiResult.data["store"]);
    //
    // return storeModel.store;
  }
  ...
  Future<bool> AddProduct({
    ...
    try {
      final apiResult =
          await postRequest(Api.addProduct, formData, includeHeaders: true);
      if (apiResult.statusCode == 200) if (apiResult.data["class"] ==
          "success") {
        return true;
      } else
