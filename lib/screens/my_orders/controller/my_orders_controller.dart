...
try{
  ...
  if(k){
    Get.back();
    Get.rawSnackbar(message: "لقد تم اضافة طلبك بنجاح ",backgroundColor: Colors.green);
  await getMyOrders();
...
update();
  ...
  deleteOrder(int id)async
  {
    try{
      ...
      if(k){
        Get.rawSnackbar(message: "لقد تم حذف طلبك بنجاح ",backgroundColor: Colors.green);
        await getMyOrders();
        await allOrdersController.fetchData();
        removeLoading();
      }
