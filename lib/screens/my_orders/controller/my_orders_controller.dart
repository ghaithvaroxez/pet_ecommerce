...
// }
  ...
  addOrder(
    ...
    try {
      setLoading();
      bool k = await myOrdersRequests.addOrder(
          orderImage, name, desc, cat_id, type_id, order_type, price);
      if (k) {
        Get.back();
        Get.rawSnackbar(
            message: "لقد تم اضافة طلبك بنجاح ".i18n,
        ...
        Get.rawSnackbar(
            message: "عذرا حدث خطأ ما الرجاء المحاولة مرة أخرى ".i18n,
  ...
  deleteOrder(int id) async {
    try {
      ...
      if (k) {
        Get.rawSnackbar(
            message: "لقد تم حذف طلبك بنجاح ".i18n,
            backgroundColor: Colors.green);
        await getMyOrders();
        await allOrdersController.fetchData();
        removeLoading();
