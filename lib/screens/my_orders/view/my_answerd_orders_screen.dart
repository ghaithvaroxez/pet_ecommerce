...
class _MyAnswerdOrdersScreenState extends State<MyAnswerdOrdersScreen> {
  ...
  fetchData() async {
    ...
    if (apiResult.statusCode == 200) {
      var j = jsonDecode(apiResult.body);
      answers = j["orders"] == null
          ? []
          : List<Answer>.from(j["orders"].map((x) => Answer.fromJson(x)));
      storeContacts = List<StoreContact>.from(
          j["socials"].map((x) => StoreContact.fromJson(x)));
      for (int i = 0; i < storeContacts.length; i++) {
        if (storeContacts[i].type == "facebook" && bfb == true) {
          fb = storeContacts[i].link;
          bfb = false;
        } else if (storeContacts[i].type == "phone" && bwa == true) {
          wa = storeContacts[i].link;
          bwa = false;
        } else if (storeContacts[i].type == "instagram" && bins == true) {
          ins = storeContacts[i].link;
          bins = false;
        }
      }
    } else {
