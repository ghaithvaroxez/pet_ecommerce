...
class SearchRequests extends HttpService {
  ...
  Future<List<Offer>> getOffers(String offerName,
      {int catId = -1, int typeId = -1}) async {
    ...
    if (apiResult.statusCode == 200) {
      return List<Offer>.from(
          apiResult.data["items"].map((x) => Offer.fromJson(x)));
    } else
      return [Offer(error: true)];
  }

  Future<List<StoreProduct>> getProducts(String productName,
      {int catId = -1,
      int typeId = -1,
      double price1 = -1,
    ...
    final apiResult = await getRequest(mainUrl);
    if (apiResult.statusCode == 200) {
      return List<StoreProduct>.from(
          apiResult.data["items"].map((x) => StoreProduct.fromJson(x)));
    } else
      return [StoreProduct(error: true)];
  }

  Future<List<Store>> getStores(String storeName,
      {int district = -1,
      bool store = false,
      bool barn = false,
    ...
    if (apiResult.statusCode == 200) {
      return List<Store>.from(
          apiResult.data["items"].map((x) => Store.fromJson(x)));
    } else
  ...
  Future<List<Doctor>> getDoctors(
    ...
    if (apiResult.statusCode == 200) {
      return List<Doctor>.from(
          apiResult.data["items"].map((x) => Doctor.fromJson(x)));
    } else
      return [Doctor(error: true)];
  }
  ...
  Future<List<Category>> getCat(
    ...
    if (apiResult.statusCode == 200) {
      return List<Category>.from(
          apiResult.data["items"].map((x) => Category.fromJson(x)));
    } else
