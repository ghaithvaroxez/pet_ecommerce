    ...
    return Scaffold(
      body: SafeArea(
        child: StoryView(
            ...
            storyItems: [
              ...List<StoryItem>.generate(
                  after.length,
                  (index) => after[index].type == "image"
                      ? StoryItem.pageImage(
                          url: Api.imagePath + after[index].image,
                          duration: Duration(seconds: after[index].duration),
                          controller: storyController)
                      : StoryItem.pageVideo(Api.imagePath + after[index].image,
                          duration: Duration(seconds: after[index].duration),
                          controller: storyController)),
              ...List<StoryItem>.generate(
                  before.length,
                  (index) => before[index].type == "image"
                      ? StoryItem.pageImage(
                          url: Api.imagePath + before[index].image,
                          duration: Duration(seconds: before[index].duration),
                          controller: storyController)
                      : StoryItem.pageVideo(Api.imagePath + before[index].image,
                          duration: Duration(seconds: before[index].duration),
                          controller: storyController)),
            ]),
      ),
