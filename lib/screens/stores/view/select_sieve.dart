...
class SelectSieveView extends StatefulWidget {
  @override
  _SelectSieveViewState createState() => _SelectSieveViewState();
}

class _SelectSieveViewState extends State<SelectSieveView> {
  bool loading =false;
  List<Store> Stores;
  bool failed=false;
  fetchData()async
  {
    ...
    if(apiResult.statusCode==200)
    {
      ...
      Stores =List<Store>.from(j["sieves"].map((x) => Store.fromJson(x)));
    }
    else {
      failed=true;
    }
                    ...
                    child: Container(
                        ...
                        child: Row(
                          children: [
                            ...
                            Spacer(),
                            Container(height:getProportionateScreenHeight(28),child: AutoSizeText("المناحل",style: h5_21pt,minFontSize: 8,)),
                            Spacer(),
                            CircleAvatar(
                              radius: 24,
                              backgroundColor: Colors.grey.shade50,
