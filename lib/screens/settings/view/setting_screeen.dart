...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  fetchData() async {
    ...
    if (apiResult.statusCode == 200) {
      ...
      if (j["store_not"] == 1) stores = true;
      if (j["offer_not"] == 1) offers = true;
      if (j["barn_not"] == 1) barns = true;
      if (j["sieve_not"] == 1) sieves = true;
      if (j["order_not"] == 1) orders = true;
    } else {
      failed = true;
    }
    loading = false;
  ...
  ChangeOffers() async {
    ...
    final h = await HttpService().getHeaders();
    final apiResult = await http
        .post(url, headers: h, body: {"offer_not": offers ? "1" : "0"});
    if (apiResult.statusCode == 200) {
    } else {
  ...
  ChangeOrders() async {
    try {
      ...
      final h = await HttpService().getHeaders();
      final apiResult = await http
          .post(url, headers: h, body: {"order_not": orders ? "1" : "0"});
      if (apiResult.statusCode == 200) {
      } else {
        Get.rawSnackbar(message: "الرجاء المحاولة مجدداً".i18n);
        setState(() {
  ...
  ChangeStores() async {
    try {
      ...
      final h = await HttpService().getHeaders();
      final apiResult = await http
          .post(url, headers: h, body: {"store_not": stores ? "1" : "0"});
      if (apiResult.statusCode == 200) {
      } else {
        Get.rawSnackbar(message: "الرجاء المحاولة مجدداً".i18n);
        setState(() {
  ...
  ChangeSieves() async {
    try {
      ...
      final h = await HttpService().getHeaders();
      final apiResult = await http
          .post(url, headers: h, body: {"sieve_not": sieves ? "1" : "0"});
      if (apiResult.statusCode == 200) {
      } else {
        Get.rawSnackbar(message: "الرجاء المحاولة مجدداً".i18n);
        setState(() {
  ...
  ChangeStable() async {
    try {
      ...
      final h = await HttpService().getHeaders();
      final apiResult = await http
          .post(url, headers: h, body: {"barn_not": barns ? "1" : "0"});
      if (apiResult.statusCode == 200) {
      } else {
        Get.rawSnackbar(message: "الرجاء المحاولة مجدداً".i18n);
        setState(() {
