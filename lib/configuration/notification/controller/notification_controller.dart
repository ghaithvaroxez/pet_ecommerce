...
class NotificationController extends GetxController{
  ...
  Future onDidReceiveLocalNotification(
    ...
    AndroidNotificationDetails(
        'pets', 'pets', 'your channel description',
  ...
  checkNotificationType(RemoteMessage message) async{
    ...
    {
      _scheduleNotification('${message.notification.title}','${message.notification.body}','${message.data['type']}',int.parse(message.data['item_id']));

      Get.snackbar('${message.notification.title}', '${message.notification.body}',duration: Duration(seconds: 5),snackStyle: SnackStyle.FLOATING,onTap:(n)=>         Get.to(OrderDetailsScreen(int.parse(message.data['item_id'])) ));
    ...
    if('${message.data['type']}'=='New Store Notification')
    {
      ...
      _scheduleNotification('${message.notification.title}','${message.notification.body}','${message.data['type']}',int.parse(message.data['item_id']));
     Get.snackbar('${message.notification.title}', '${message.notification.body}',duration: Duration(seconds: 5),snackStyle: SnackStyle.FLOATING,onTap:(n)=>       Get.to(StoreDetailsById(int.parse(message.data['item_id'])) ));

    }
    else{
      _scheduleNotification('${message.notification.title}','${message.notification.body}',"",-1);
      Get.snackbar('${message.notification.title}', '${message.notification.body}',duration: Duration(seconds: 5),snackStyle: SnackStyle.FLOATING, );
    }
  }
